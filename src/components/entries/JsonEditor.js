...
export default function JsonEditorEntry(props) {
  ...
  const [ localDiagnostic, setLocalDiagnostic ] = useState(
    () => computeDiagnostic(validate, value, translate)
  );
  ...
  useEffect(() => {
    ...
    setLocalDiagnostic(computeDiagnostic(validate, value, translate));
  }, [ value, validate ]);
  ...
  const onInput = useStaticCallback((newValue) => {
    ...
    if (newValue !== currentValue) {
      ...
      setLocalDiagnostic(computeDiagnostic(validate, newValue, translate));
    }
  });
  ...
  return (
    <div
      class={ classnames('bio-properties-panel-entry', ENTRY_SEVERITY_CLASS[ diagnostic?.severity ]) }
      data-entry-id={ id }>
      <JsonEditor
        id={ id }
        key={ element }
...
function computeDiagnostic(validate, value, translate) {
  const custom = isFunction(validate) ? validate(value) : null;
